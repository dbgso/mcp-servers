...
### Name a Shared Params Object

The `single-params-object` rule requires a single object parameter. When the
**same object shape is used by two or more signatures**, extract it into a named
type instead of repeating the inline literal. This keeps the shape DRY and
forces related handlers to stay in lockstep — add a field once and every
signature gets it.

```typescript
// ❌ Bad - same shape repeated inline across signatures
export interface Operation<TArgs, TCtx> {
  preview?: (params: { args: TArgs; ctx: TCtx }) => Promise<string> | string;
  execute: (params: { args: TArgs; ctx: TCtx }) => Promise<ToolResponse>;
}

// ✅ Good - named once, both signatures reference it
...
A one-off params object used by a single signature stays inline — naming it adds
no value. The trigger is **reuse across signatures**, not object params in general.
