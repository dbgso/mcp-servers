...
${task.content}`;
  ...
  async approveTask(id: string): Promise<{ success: boolean; error?: string; autoCompletedParents?: string[] }> {
    const task = await this.getTask(id);
    if (!task) {
      return { success: false, error: `Task "${id}" not found.` };
    }
    ...
    // Auto-complete parent if all children are finished
    const autoCompletedParents = await this.autoCompleteParents(id);

    return { success: true, autoCompletedParents };
  }

  /**
   * Recursively auto-complete parent tasks when all children are finished.
   * Returns list of parent task IDs that were auto-completed.
   */
  private async autoCompleteParents(childId: string): Promise<string[]> {
    const completedParents: string[] = [];
    const task = await this.getTask(childId);
    if (!task || !task.parent) {
      return completedParents;
    }

    const parent = await this.getTask(task.parent);
    if (!parent) {
      return completedParents;
    }

    // Skip if parent is already finished
    if (this.isTaskFinished(parent.status)) {
      return completedParents;
    }

    // Check if all children of parent are finished
    const siblings = await this.getChildTasks(parent.id);
    const allFinished = siblings.every((sibling) =>
      this.isTaskFinished(sibling.status)
    );
    ...
    const parentPath = this.idToPath(parent.id);
    await fs.writeFile(parentPath, this.serializeTask(updatedParent), "utf-8");
    this.invalidateCache();
    completedParents.push(parent.id);

    // Recursively check grandparent
    const grandparentCompleted = await this.autoCompleteParents(parent.id);
    completedParents.push(...grandparentCompleted);
