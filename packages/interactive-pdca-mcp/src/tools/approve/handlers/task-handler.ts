...
\`approve(target: "task", task_id: "${task_id}", approvalToken: "<token>")\``,
    ...
    // Show auto-completed parent tasks
    let autoCompleteInfo = "";
    if (result.autoCompletedParents && result.autoCompletedParents.length > 0) {
      autoCompleteInfo = `\n\n**Auto-completed parent tasks:**\n${result.autoCompletedParents.map((id) => `- ${id}`).join("\n")}`;
    }
    ...
    return {
      content: [
        {
          type: "text" as const,
          text: `Task "${task_id}" approved and marked as completed.\n\nOutput was: ${task.output}${autoCompleteInfo}${additionalInfo}${reviewFilesInfo}`,
        },
      ],
    };
  }
