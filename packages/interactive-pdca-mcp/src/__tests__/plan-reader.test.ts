...
describe("PlanReader", () => {
  ...
  describe("approveTask", () => {
    ...
    it("should auto-complete parent when all children are completed", async () => {
      // Create parent task
      await reader.addTask({
        id: "parent",
        title: "Parent Task",
      ...
      // Create child tasks (simulating PDCA subtasks)
      await reader.addTask({
        id: "parent__plan",
        title: "Plan Phase",
        content: "",
      ...
      await reader.addTask({
        ...
        content: "",
        parent: "parent",
        dependencies: ["parent__plan"],
        dependency_reason: "Must complete plan first",
        prerequisites: "",
        completion_criteria: "",
        deliverables: [],
        is_parallelizable: false,
      ...
      // Start parent (moves to in_progress)
      await reader.updateStatus({ id: "parent", status: "in_progress" });

      // Complete first child
      await reader.updateStatus({ id: "parent__plan", status: "completed", output: "Done" });
      await reader.confirmSelfReview("parent__plan");
      const result1 = await reader.approveTask("parent__plan");

      expect(result1.success).toBe(true);
      // Parent should still be in_progress (not all children done)
      let parent = await reader.getTask("parent");
      expect(parent?.status).toBe("in_progress");
      expect(result1.autoCompletedParents).toEqual([]);

      // Complete second child
      await reader.updateStatus({ id: "parent__do", status: "completed", output: "Done" });
      await reader.confirmSelfReview("parent__do");
      const result2 = await reader.approveTask("parent__do");

      expect(result2.success).toBe(true);
      // Parent should now be auto-completed
      expect(result2.autoCompletedParents).toEqual(["parent"]);

      parent = await reader.getTask("parent");
      expect(parent?.status).toBe("completed");
    });

    it("should recursively auto-complete grandparent", async () => {
      ...
      // Start all tasks
      await reader.updateStatus({ id: "grandparent", status: "in_progress" });
      await reader.updateStatus({ id: "parent", status: "in_progress" });

      // Complete child
      await reader.updateStatus({ id: "child", status: "completed", output: "Done" });
      await reader.confirmSelfReview("child");
      const result = await reader.approveTask("child");

      expect(result.success).toBe(true);
      // Both parent and grandparent should be auto-completed
      expect(result.autoCompletedParents).toEqual(["parent", "grandparent"]);

      const parent = await reader.getTask("parent");
      const grandparent = await reader.getTask("grandparent");
      expect(parent?.status).toBe("completed");
      expect(grandparent?.status).toBe("completed");
    });

    it("should not auto-complete parent if sibling is not finished", async () => {
      await reader.addTask({
        id: "parent",
        title: "Parent",
        content: "",
      ...
      await reader.updateStatus({ id: "child1", status: "completed", output: "Done" });
      await reader.confirmSelfReview("child1");
      const result = await reader.approveTask("child1");

      expect(result.success).toBe(true);
      expect(result.autoCompletedParents).toEqual([]);
    ...
    it("should count skipped siblings as finished for auto-complete", async () => {
      await reader.addTask({
        id: "parent",
        title: "Parent",
        content: "",
      ...
      await reader.updateStatus({ id: "parent", status: "in_progress" });
      // Skip child2
      await reader.updateStatus({ id: "child2", status: "skipped" });
      ...
      expect(result.success).toBe(true);
      // Parent should be auto-completed (child2 is skipped, counts as finished)
      expect(result.autoCompletedParents).toEqual(["parent"]);
