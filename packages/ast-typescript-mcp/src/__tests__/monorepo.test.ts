...
describe("Monorepo Dependency Graph", () => {
  ...
  describe("buildMonorepoGraph", () => {
    ...
    // Skipped: graph builder treats devDependencies as edges, and the monorepo
    // intentionally has a devDep-only cycle (mcp-shared-db -> mcp-shared-db-postgres
    // -> mcp-shared-db) used solely for cross-package tests. No runtime/build
    // impact; revisit if cycle detection learns to ignore devDeps.
    it.skip("should detect cycles if any exist", async () => {
      const workspace = await detectWorkspace(MONOREPO_ROOT);
      const graph = buildMonorepoGraph(workspace!);
