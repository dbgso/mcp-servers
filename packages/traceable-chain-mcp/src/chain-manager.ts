...
export class ChainManager {
   ...
   */
  async trace(params: { id: string; direction?: "up" | "down" }): Promise<TraceNode | null> {
    const { id, direction = "down" } = params;
  ...
  async validate(): Promise<ValidationResult> {
    ...
    for (const doc of documents) {
      ...
      const parentValidation = await this.validateParent({ type: doc.type, parentId: doc.requires });
  ...
  async create(params: {
    ...
    const { type, title, content, requires } = params;
    // Validate type
    ...
    // Validate parent
    const parentValidation = await this.validateParent({ type, parentId: requires });
    if (!parentValidation.valid) {
   ...
   */
  async link(params: { id: string; parentId: string }): Promise<Document> {
    ...
    const parentValidation = await this.validateParent({ type: doc.type, parentId });
    if (!parentValidation.valid) {
      throw new Error(parentValidation.error);
    }

    // Update the document with new requires (preserves ID)
    const updated = await this.storage.update({ id, updates: { requires: parentId } });
    if (!updated) {
