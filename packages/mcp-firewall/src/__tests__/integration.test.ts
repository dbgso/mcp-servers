...
// The approval token is delivered out-of-band (desktop notification) and is
// never written to disk, so this black-box test — which spawns the server as a
...
describe("mcp-firewall Ask Action", () => {
  ...
  it("should successfully approve with the valid out-of-band token", async () => {
    // Trigger an ask rule
    const askResult = await client.callTool({
      name: "proxy_execute",
    ...
    // The human relays the token received out-of-band (desktop notification);
    // it is never on disk. Here it is pinned via MCP_APPROVAL_TEST_TOKEN.
    const approveResult = await client.callTool({
      name: "proxy_approve",
      arguments: { requestId, approvalToken: APPROVAL_TOKEN },
    });
