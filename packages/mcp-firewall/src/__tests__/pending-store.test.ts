...
describe("PendingStore", () => {
  ...
  describe("add", () => {
    test("should add pending call with expiration", () => {
      ...
      const pending = store.add({ toolName: "browser_click", args: { ref: "btn-1" }, matchedRule: mockRule });
  ...
  describe("list", () => {
    test("should return only non-expired calls", () => {
      store.add({ toolName: "tool1", args: {}, matchedRule: mockRule });
      vi.advanceTimersByTime(6 * 60 * 1000);

      store.add({ toolName: "tool2", args: {}, matchedRule: mockRule });
  ...
  describe("cleanup", () => {
    test("should remove all expired calls", () => {
      store.add({ toolName: "tool1", args: {}, matchedRule: mockRule });
      store.add({ toolName: "tool2", args: {}, matchedRule: mockRule });
      store.add({ toolName: "tool3", args: {}, matchedRule: mockRule });
