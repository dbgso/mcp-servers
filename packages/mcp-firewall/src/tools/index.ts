...
import { validateApproval, clearApproval } from "mcp-shared/approval";
import { z } from "zod";
...
export function registerRuleTools(params: RegisterRuleToolsParams): void {
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      try {
        const result = await proxyClient.callTool({ name: pending.toolName, args: pending.args });
